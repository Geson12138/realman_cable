...
class Kinematics:
    ...
    def inverse_kinematics(
        ...
        if method == "NR":
            joints = self._compute_IK_NR(
                frames, current_joints, target_pose, max_iter=max_iter
            )
        if method == "LM":
        ...
        if method == "GaBO":
            joints = self._compute_IK_GaBO(
                frames, target_pose, max_iter=max_iter, opt_dimension=2
            )
        return joints
    ...
    def _compute_IK_NR(self, frames, current_joints, target_pose, max_iter):
        """
    ...
    def _compute_IK_LM(self, frames, current_joints, target_pose, max_iter):
        """
    ...
    def _compute_IK_LM2(self, frames, current_joints, target_pose, max_iter):
        """
        Computes inverse kinematics using Levenberg-Marquatdt method
    ...
    def _compute_IK_GaBO(
        ...
        try:
            ...
            print(
                "ImportError: No module found to run GaBO IK method. Try install requirements"
            )
        ...
        while True:
            x_init = np.array(
                [
                    np.array(robot_manifold.rand()).reshape(2 * robot_dimension)
        ...
        mll_fct, model, solver, bounds, constraints = g_util.init_gp_model(
            opt_dimension, device, x_data, y_data
        )
        ...
        for iteration in range(nb_iter_bo):
            ...
            acq_fct = ExpectedImprovement(
                model=model, best_f=best_f[-1], maximize=False
            )
            acq_fct.to(device)

            # Get new candidate
            new_x = joint_optimize_manifold(
                acq_fct,
                opt_manifold,
                solver,
                q=1,
            ...
            model.set_train_data(
                x_data, y_data, strict=False
            )  # strict False necessary to add datapoints
            print(
                "Iteration "
                + str(iteration)
                + "\t Best error "
...
class Baxter:
    left_e0_fixed_offset = Transform(rot=[0.5, 0.5, 0.5, 0.5], pos=[0.107, 0.0, 0.0])
    left_w0_fixed_offset = Transform(rot=[0.5, 0.5, 0.5, 0.5], pos=[0.088, 0.0, 0.0])
    right_e0_fixed_offset = Transform(rot=[0.5, 0.5, 0.5, 0.5], pos=[0.107, 0.0, 0.0])
    right_w0_fixed_offset = Transform(rot=[0.5, 0.5, 0.5, 0.5], pos=[0.088, 0.0, 0.0])

    @staticmethod
    def add_visual_link(link_transforms, f):
        if "left_lower_shoulder" in f.link.name:
            link_transforms["left_upper_elbow_visual"] = np.dot(
                link_transforms["left_lower_shoulder"], Baxter.left_e0_fixed_offset
            )
        if "left_lower_elbow" in f.link.name:
            link_transforms["left_upper_forearm_visual"] = np.dot(
                link_transforms["left_lower_elbow"], Baxter.left_w0_fixed_offset
            )
        if "right_lower_shoulder" in f.link.name:
            link_transforms["right_upper_elbow_visual"] = np.dot(
                link_transforms["right_lower_shoulder"], Baxter.right_e0_fixed_offset
            )
        if "right_lower_elbow" in f.link.name:
            link_transforms["right_upper_forearm_visual"] = np.dot(
                link_transforms["right_lower_elbow"], Baxter.right_w0_fixed_offset
            )
