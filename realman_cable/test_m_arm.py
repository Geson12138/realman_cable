...
def get_target_xyz():
    global point6_pose
    pose_out = np.eye(4)
    # pose_out[0,3] = point6_pose.poses[0].position.x
    # pose_out[1,3] = point6_pose.poses[0].position.y
    # pose_out[2,3] = point6_pose.poses[0].position.z
    # print(pose_out)
    
    err_code, joint_l, pose_l, arm_err_ptr, sys_err_ptr = arm_le.Get_Current_Arm_State()
    ee_rotateMatrix = SO3.RPY(pose_l[3], pose_l[4], pose_l[5])
    ee2base_transform_matrix = np.eye(4)
    ee2base_transform_matrix[:3, :3] = ee_rotateMatrix
    ee2base_transform_matrix[:3, 3] = [pose_l[0], pose_l[1], pose_l[2]]
    print('当前base下夹爪的旋转矩阵', np.array(ee_rotateMatrix))
    ################################################################
...
if __name__ == '__main__':
    ...
    #####################################################################
    rospy.Subscriber("/left_camera_predict_usb_pointXYZ", PoseArray, point6_pose_callback)
